...
| `REQ_AUTH_METHOD`          | Authentication method to use for requests to `REQ_URL` and for `*.url` triggered requests. Options are `basic` for basic authentication, `kubernetes_service_account` for authentication with pod service account token and `none` for no authentication. If not set, it will be determined based on the presence of `REQ_USERNAME`/`REQ_PASSWORD` or their file-based counterparts. | false | - | string  |
