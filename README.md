...
| `FOLDER_PER_NAMESPACE`     | Set to `true` to place files in a subfolder of `FOLDER` named after the resource's namespace. If `FOLDER_ANNOTATION` is also set and provides a relative path, that relative path is placed under the namespace subfolder as well.                                                                                                  | false    | `false`                                   | boolean |
